...
class CreateLog ():
    ...
    def UpdateUser(self):
        ...
        if self.username_entry.get()=="" or self.email_entry.get()==""  or self.lastname_entry.get()=="" or self.question_entry.get()=="" or self.reponse_entry.get()=="" or self.password_entry.get()=="" or self.Cfpassword_entry.get()=="" :
            messagebox.showerror("Modification", "Héros, nous avons besoin de tous vos objets magiques.",parent=self.zone1)
        # On se rassure également que la confirmation de mot de passe est conforme également
            ...
            messagebox.showerror("Modification","Vos clés magiques ne sont pas conformes.",parent=self.root)
        else:
                ...
                try:
                    ...
                    if row_MDB != None :
                        ...
                        messagebox.showinfo("Modification","Vos objets magiques ont été actualisés avec succès.",parent=self.root)
                        ...
                        messagebox.showinfo("Modification", f"Un email a été envoyé à {receiver_email} avec votre clé magique modifiée, vous en aurez besoin.",parent=self.root)
                        self.ReinitBox() # On réinitialise les textboxes  
                    else:
                        messagebox.showinfo("Modification", "Désolé mais nous ne parvenons pas à trouver cet héros parmi nos guerriers.",parent=self.root) 

                    client.close()
                    self.root.destroy()
                    
                except Exception as ex:
                    messagebox.showerror("Connexion",f"Oups! Nous rencontrons un problème pour ouvrir le monde magique: {str(ex)}",parent=self.root)       
    ...
    def CreateUser(self):
        ...
        if self.username_entry.get()=="" or self.email_entry.get()=="" or self.lastname_entry.get()=="" or self.question_entry.get()=="" or self.reponse_entry.get()=="" or self.password_entry.get()=="" or self.Cfpassword_entry.get()=="" :
            messagebox.showerror("Création","Héros, nous avons besoin de tous vos objets magiques.",parent=self.zone1)
        # On vérifie le mot de passe saisi est conforme au mdp confirmation
        elif self.password_entry.get()!= self.Cfpassword_entry.get():
            messagebox.showerror("Création","Vos clés magiques ne sont pas conformes.",parent=self.root)
        else:
                ...
                try:
                    ...
                    if row_MDB == None : # Si l'email saisi n'existe pas
                        ...
                        if resultat.inserted_id:
                            messagebox.showinfo("Création", f"Bienvenue jeune héro vous êtes classé au numéro: {resultat.inserted_id}.",parent=self.root)                    
                            # Définir les détails de l'e-mail
                            sender_email = 'sorolassina58@gmail.com'
                            sender_password ="mxcu kxhv jwym staa"
                            receiver_email = self.email_entry.get()
                            ...
                            messagebox.showinfo("Création", "Oups! Désolé jeune héros une barrière semble nous empêcher de vous créer.",parent=self.root)

                        client.close() # On ferme la connexion au serveur par prudence    
                        self.root.destroy() # On ferme le formulaire de création
                    else:
                        messagebox.showinfo("Création", "Oups! Quelqu'un possède déjà cet objet magique.",parent=self.root)

                    #client.close() # On ferme la connexion au serveur par prudence
                    #self.root.destroy() # On ferme le formulaire de création                       
                except Exception as ex:
                    messagebox.showerror("Création",f"Oups! Nous rencontrons un problème pour nous ouvrir le monde magique : {str(ex)}",parent=self.root)       
    ...
    def load_background_image(self):       
        background_image_pil = Image.open("./images/BG3.jpg")
